...
import { WORLDS_2026_MEN, WORLDS_2026_WOMEN } from "./worlds2026";

export const RACES = {
  [TDF_2026.slug]: TDF_2026,
  [VUELTA_2026.slug]: VUELTA_2026,
  [WORLDS_2026_MEN.slug]: WORLDS_2026_MEN,
  [WORLDS_2026_WOMEN.slug]: WORLDS_2026_WOMEN,
};

// The race the site shows by default (homepage, /predictions, /leaderboard...).
// Everything else stays reachable as an archive.
export const ACTIVE_RACE_SLUG = WORLDS_2026_MEN.slug;
...
// A championship is a week of separate one-day events rather than a single
// race with a general classification. Anything that assumes consecutive
// stages, cumulative progress or jerseys should switch on this.
export function isChampionship(race) {
  return race.type === "championship";
}

// True when the race has a general classification running through it, which is
// what makes "stage 4 of 21", overall progress and jersey pages meaningful.
export function hasOverallClassification(race) {
  return race.type === "grand-tour" || race.type === "stage-race";
}
...
export function raceFromPathname(pathname) {
  ...
  // A race may claim a friendlier URL prefix than its slug -- the women's
  // championship lives under /women rather than /worlds-2026-women.
  for (const seg of segments) {
    const byPrefix = allRaces().find(
      (r) => r.urlPrefix && r.urlPrefix.replace(/^\//, "") === seg
    );
...
// Where a race's pages live. The active race owns the root URLs; anything else
// sits under its own prefix (or slug, if it has no prefix).
export function raceBasePath(race, lang) {
  const langPrefix = lang === "es" ? "/es" : "";
  if (race.slug === ACTIVE_RACE_SLUG) return langPrefix;
  if (race.urlPrefix) return langPrefix + race.urlPrefix;
  return "/" + race.slug;
}

// The men's and women's editions of the same championship, so the UI can offer
// a category switch between them.
export function siblingCategories(race) {
  if (!race.category) return [];
  return allRaces().filter(
    (r) => r.year === race.year && r.type === race.type && r.host && race.host &&
           r.host.city === race.host.city
  );
}
